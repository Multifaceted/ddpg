...
class DDPG:
    def __init__(self, 
        ...
        assert n_states > 0 and isinstance(n_states, int)
        assert n_actions > 0 and isinstance(n_actions, int)
        assert lr_actor > 0 and isinstance(lr_actor, float)
        assert lr_critic > 0 and isinstance(lr_critic, float)
    ...
    def act(self, states, base, is_train=True, requires_grad=True, override=None, epsilon_0=.5, beta=1E-4 *3, dynamic_explore=True):
        ...
        if self.lastReward < 1E-2 and self.steps < 100000:   #TODO change back
            explore_normal = True
        ...
        if explore_uniform:
            ...
            if not requires_grad:
                with torch.no_grad():
                    actions = self.actor_source(states)
                    if explore_normal:
                        actions += torch.normal(mean=torch.tensor(0.), std=torch.tensor(1.))
    ...
    def update(self, writer, round, loss_fun=nn.MSELoss()):
        """
        ...
        writer.add_scalar("CriticLoss"+str(self.index), loss_critic.item(), round)
        ...
        writer.add_scalar("ActorLoss"+str(self.index), loss_actor.item(), round)

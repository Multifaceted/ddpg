...
class Actor(nn.Module):
    def __init__(self, value, n_states=5, n_actions=1, bidMin=.2, bidMax=5, hidden1=200, hidden2=100, constrain=True):
        """
    ...
    def forward(self, states):
        ...
        assert states.type() == 'torch.FloatTensor'
...
class Critic(nn.Module):
    ...
    def forward(self, states, actions):
        ...
        assert states.type() == 'torch.FloatTensor'
        assert actions.type() == 'torch.FloatTensor'
        assert states.shape[-1] == self.n_states
...
class Memory:
    ...
    def append(self, states_old, actions, states_new, reward, epoch):
        ...
        """
        assert states_old.type() == 'torch.FloatTensor'
        assert actions.type() == 'torch.FloatTensor'
        assert states_new.type() == 'torch.FloatTensor'
        assert states_old.shape[-1] == self.n_states
        assert states_new.shape[-1] == self.n_states
        assert reward.dim() == 1
        assert reward.type() == 'torch.FloatTensor'
...
class DDPG:
    ...
    def act(self, states, is_train=True, requires_grad=True, override=None, epsilon_0=.5, beta=1E-3):
        """
        Make a bid.
        """
        assert states.type() == "torch.FloatTensor"
        ...
        # if self.steps < 50:
        #      actions -= is_train * (torch.rand(size=(1, )) - .5) * 20
        # else:
        #     actions -= is_train * (torch.rand(size=(1, )) - .5) * 20 * torch.exp(torch.tensor(-(self.steps-50) / 5) ) * 1
        

        if torch.rand(size=(1,)).item() < epsilon_0 * (torch.exp(torch.tensor(-beta * self.steps))).item():
    ...
    def reset(self):
        ...
        # self.actor_source.to(device)
        # self.actor_target.to(device)
        # self.critic_source.to(device)
